...
const CHART_FALLBACK = String(process.env.NEXT_PUBLIC_CHART_FALLBACK_OLTP || "false") === "true";
...
export class ChartRepoSupabase implements ChartRepo {
  async fetchMetricsDay(range: RangeOption, metrics?: Metric[]): Promise<ChartVM> {
    if (!CHART_FALLBACK) return this.readFromChartDB(range, metrics);
  ...
  async fetchTimeline(range: RangeOption, metrics?: Metric[], cursor?: string | null): Promise<TimelineVM> {
    if (!CHART_FALLBACK) return this.readTimelineFromChartDB(range, metrics, cursor);
  ...
  private async readFromChartDB(range: RangeOption, _metrics?: Metric[]): Promise<ChartVM> {
    ...
    const { data, error } = await sb
      .from("metrics_day")
      .select("date, bg_avg, bp_sys_avg, bp_dia_avg, weight_kg, water_ml, insulin_units, meals_count")
      .gte("date", start)
      .lte("date", end)
