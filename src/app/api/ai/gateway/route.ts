...
import { buildCompressedContext, CompressedAIContext } from "@/modules/ai/context";
...
const AI_MODEL_DEFAULT = process.env.AI_MODEL_DEFAULT || "gpt-5-nano";
const AI_MAXTOKENS_DEFAULT = parseInt(process.env.AI_MAXTOKENS_DEFAULT || "60");
const AI_MAXTOKENS_SAFETY = parseInt(process.env.AI_MAXTOKENS_SAFETY || "120");
const AI_DISABLE_RETRY = process.env.AI_DISABLE_RETRY === "true";
const AI_CACHE_ENABLED = process.env.AI_CACHE_ENABLED !== "false"; // Default enabled
const AI_CACHE_TTL_SHORT_MIN = parseInt(process.env.AI_CACHE_TTL_SHORT_MIN || "60");
const AI_CACHE_TTL_LONG_MIN = parseInt(process.env.AI_CACHE_TTL_LONG_MIN || "1440");
const AI_RULES_FALLBACK_ENABLED = process.env.AI_RULES_FALLBACK_ENABLED !== "false"; // Default enabled
const AI_BUDGET_ENABLED = process.env.AI_BUDGET_ENABLED === "true";
const AI_BUDGET_DAILY_TOKENS = parseInt(process.env.AI_BUDGET_DAILY_TOKENS || "300000");
const AI_BUDGET_DROP_ON_EXCEEDED = process.env.AI_BUDGET_DROP_ON_EXCEEDED !== "false"; // Default enabled
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let currentIntent: Intent = body.intent || classifyIntent(body.message || "");
    ...
    // 6) Budget guard
    if (AI_BUDGET_ENABLED) {
      const dailyUsedTokens = getDailyTokenUsage(body.user_id);
      if (dailyUsedTokens + approxTokens > AI_BUDGET_DAILY_TOKENS) {
        if (AI_BUDGET_DROP_ON_EXCEEDED || ["simple_qa", "meal_tip", "reminder_reason"].includes(currentIntent)) {
          const response = {
            request_id: runId,
            ts: Date.now(),
            model: "rule-based",
