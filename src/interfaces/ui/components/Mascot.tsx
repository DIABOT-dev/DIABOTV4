...
export default function Mascot({ state = 'idle', onClose }: MascotProps) {
  ...
  return (
    <div className="fixed bottom-24 right-4 z-50 max-w-xs">
      <div className={`${mascotContent.bgColor} ${mascotContent.borderColor} border-2 rounded-2xl shadow-lg p-4 relative animate-bounce-gentle`}>
        {/* Close button */}
        <button
          onClick={handleClose}
        ...
        <div className="absolute bottom-0 right-8 transform translate-y-full">
          <div className={`w-0 h-0 border-l-8 border-r-8 border-t-8 border-l-transparent border-r-transparent ${
            currentState === 'alert' ? 'border-t-red-200' :
            currentState === 'reminder' ? 'border-t-blue-200' :
            currentState === 'meal' ? 'border-t-green-200' :
            currentState === 'report' ? 'border-t-blue-200' :
            'border-t-gray-200'
          }`}></div>
        </div>
      </div>
